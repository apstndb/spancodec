...
var (
	...
	ErrNotStructPointer = errors.New("spancodec: destination must be a non-nil pointer to a struct")
	...
	ErrNoMatchingField = errors.New("spancodec: no corresponding struct field for column")
	...
	ErrNullStructElement = errors.New("spancodec: NULL STRUCT element cannot be decoded into a non-pointer struct slice")
)
...
func decodeCustomSlice(cfg decodeConfig, gcv spanner.GenericColumnValue, dst reflect.Value) error {
	...
	if lv == nil {
		return fmt.Errorf("spancodec: ARRAY value carries %T, not a list value", gcv.Value.GetKind())
	}
	...
	for i, v := range values {
		...
		if err := decode(egcv, out.Index(i).Addr().Interface(), cfg); err != nil {
			return fmt.Errorf("spancodec: element %d: %w", i, err)
		}
	}
	dst.Set(out)
	return nil
