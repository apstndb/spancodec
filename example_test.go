...
func ExampleStructColumns() {
	...
	columns, err := spancodec.StructColumns[Singer]()
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleStructColumnsAndValues() {
	...
	names, values, err := spancodec.StructColumnsAndValues(Singer{SingerID: 1, FirstName: "Marc"})
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleStructColumns_tagOptions() {
	...
	columns, err := spancodec.StructColumns[Row]() // row-shaped: options parsed
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	typ, err := spancodec.TypeFor[Row]() // STRUCT-shaped: raw tag, like the client
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleMutationColumnsAndValues() {
	...
	cols, vals, err := spancodec.MutationColumnsAndValues(
		...
		spancodec.WithColumns("SingerId", "LastName"),
	)
	if err != nil {
		fmt.Println(err)
		return
...
func ExampleRowEncoder_Rows() {
	...
	enc := spancodec.MustNewRowEncoder[variable]()
...
// injected encoder. Returning spancodec.ErrFallthrough would defer a value to
// the built-in encoding instead.
func ExampleWithValueEncoder() {
	gcv, err := spancodec.ValueOf(uint32(42),
