...
func WithValueEncoder[T any](f func(T) (spanner.GenericColumnValue, error)) EncodeOption {
	...
	return func(cfg *encodeConfig) {
		if t.Kind() == reflect.Interface {
			panic(fmt.Sprintf("spancodec.WithValueEncoder: interface type %v cannot match a dynamic type; register concrete types instead", t))
		}
...
// (notably [WithValueEncoder] / [WithGoType], whose registrations then
// inform [RowEncoder.RowType] and [RowEncoder.ResultSetMetadata]).
type RowEncoderOption interface {
	applyRowEncoder(*rowEncoderConfig)
}

type rowEncoderConfig struct {
	mask   columnMaskConfig
	encode encodeConfig
}

func (o EncodeOption) applyRowEncoder(c *rowEncoderConfig)     { o(&c.encode) }
func (o ColumnMaskOption) applyRowEncoder(c *rowEncoderConfig) { o(&c.mask) }
